...
import whop
from config import ADMIN_TELEGRAM_ID, TELEGRAM_BOT_TOKEN, TELEGRAM_PREMIUM_CHAT_ID
...
async def approve_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(TELEGRAM_PREMIUM_CHAT_ID, '<b>📡 NEURAL STRIKES</b>\n\n' + _format_signal(candidate), parse_mode='HTML')
    except Exception:
        log.exception('approved signal channel publication failed')
        await update.message.reply_text('<b>APPROVED</b>\n\nSetup tersimpan, tetapi publikasi ke Premium Channel gagal. Tidak ada sinyal baru yang dikirim oleh jalur otomatis.', parse_mode='HTML')
        return
    await update.message.reply_text('<b>APPROVED → CHANNEL</b>\n\nSetup telah melalui human approval dan berhasil dipublikasikan ke Premium Channel.', parse_mode='HTML')
...
def build_application() -> Application:
    ...
    for command, handler in [('start', start), ('premium', premium), ('link', link_cmd), ('status', status), ('signal', signal_cmd), ('help', help_cmd), ('approve', approve_cmd)]: app.add_handler(CommandHandler(command, handler))
