...
class FulfillmentOpsTests(unittest.TestCase):
    ...
    def test_same_event_retry_and_duplicate_event(self):
        ...
        exp1 = _expiry(201)
        # duplicate event (different event_id, same payment) must be ignored
    ...
    # 3 + 4: crash after claim -> recovery worker memulihkan
    def test_recovery_worker_recovers_crashed_claim(self):
        ...
        self.assertTrue(cid)  # crash terjadi DI SINI (tidak ada fulfill)
    ...
    # 5 + 6: fencing worker A vs B, slow worker after reclaim
    ...
    def test_refund_revokes_only_matching_order(self):
        ...
        # refund pembelian PERTAMA: user.token sudah = hash kedua -> revoke pertama tidak mempengaruhi
        self.assertFalse(whop_storage.revoke_order_access("ops_9a"))
        self.assertTrue(database.get_user_by_telegram_id(206).is_active)
        # refund pembayaran AKTIF terakhir -> akses dicabut
    ...
    def test_notify_failure_is_isolated(self):
        ...
        asyncio.run(wh.notify_customer(ExplodingBot(), 207, "XAU-NEURAL-TEST", 7, "ops_10"))
        # entitlement TETAP aktif — kegagalan notifikasi terisolasi (order ditandai active_notification_failed)
    ...
    # recovery worker: exhausted -> alert payload
    def test_recovery_exhausted_reports_alert_payload(self):
        whop_storage.create_order("ops_11", 208, "plan_Yc1JnCIP8jgII", 14)
        for _ in range(3):
            cid = whop_storage.claim_fulfillment("pay_ops_11", "ops_11", stale_minutes=0)
            whop_storage.record_fulfillment_failure("pay_ops_11", "boom")
        report = wh.recover_stale_fulfillments(stale_minutes=0, max_attempts=3)
        self.assertFalse(any(r["payment_id"] == "pay_ops_11" for r in report["recovered"]))  # attempts >= max -> tidak di-retry
