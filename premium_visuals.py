...
def _pending_modules(update) -> str:
    return '\n'.join([f'  {_t(update, 'price')} — XAU/USD', f'  {_t(update, 'signal')}', f'  {_t(update, 'analysis')}', f'  {_t(update, 'account')}'])

def home_keyboard(update):
    return InlineKeyboardMarkup([[InlineKeyboardButton(f'📈 {_t(update, 'price')}', callback_data='screen:price'), InlineKeyboardButton(f'🧠 {_t(update, 'signal')}', callback_data='screen:signal')], [InlineKeyboardButton(f'📊 {_t(update, 'analysis')}', callback_data='screen:analysis'), InlineKeyboardButton(f'👑 {_t(update, 'account')}', callback_data='screen:account')], [InlineKeyboardButton(f'⚙️ {_t(update, 'settings')}', callback_data='screen:settings'), InlineKeyboardButton(f'🌐 {_t(update, 'language')}', callback_data='settings:language')], [InlineKeyboardButton(f'💎 {_t(update, 'access')}', callback_data='screen:access')], [InlineKeyboardButton(_t(update, 'menu'), callback_data='nav:menu')]])

def access_keyboard(update):
    import phase2_bot
    telegram_id = update.effective_user.id
    return InlineKeyboardMarkup([[InlineKeyboardButton(f'🟢 {_days_label(update, 7)}', url=phase2_bot.checkout_link(telegram_id, 7))], [InlineKeyboardButton(f'🟡 {_days_label(update, 14)}', url=phase2_bot.checkout_link(telegram_id, 14))], [InlineKeyboardButton(f'🔵 {_days_label(update, 30)}', url=phase2_bot.checkout_link(telegram_id, 30))], [InlineKeyboardButton(_t(update, 'activate'), callback_data='action:token'), InlineKeyboardButton(_t(update, 'paid'), callback_data='paid:menu')], [InlineKeyboardButton(_t(update, 'back'), callback_data='nav:home'), InlineKeyboardButton(_t(update, 'menu'), callback_data='nav:home')]])

def price_keyboard(update):
    import main
    if _active(update):
        return main._original_price_keyboard(update)
    return InlineKeyboardMarkup([[InlineKeyboardButton(f'🟢 {_days_label(update, 7)}', url=_checkout(update, 7))], [InlineKeyboardButton(f'🟡 {_days_label(update, 14)}', url=_checkout(update, 14))], [InlineKeyboardButton(f'🔵 {_days_label(update, 30)}', url=_checkout(update, 30))], [InlineKeyboardButton(_t(update, 'back'), callback_data='screen:access'), InlineKeyboardButton(_t(update, 'menu'), callback_data='nav:home')]])
...
async def render_access(update, context):
    ...
    if active:
        ...
        text = f'<b>[ CLEARANCE ]: NEURAL GOLD v3.2</b>\n{ts.stamp()}\n{main.DIVIDER}\n\n[ ACCESS ]: {main.t(lang, 'premium_active')}\n\nCLEARANCE: <b>🟢 {main.t(lang, 'active')}</b>\n\n{main.t(lang, 'access_until').upper()}: <code>{expiry_text}</code>\n\n[ CORE ]: {main.t(lang, 'home_pitch')}'
        kb = InlineKeyboardMarkup([[InlineKeyboardButton(f'👑 {main.t(lang, 'account')}', callback_data='screen:account')], [InlineKeyboardButton(main.t(lang, 'menu'), callback_data='nav:menu')]])
    else:
        text = f'<b>[ CLEARANCE ]: NEURAL GOLD v3.2</b>\n{ts.stamp()}\n{main.DIVIDER}\n\n[ ACCESS ]: PENDING // OPERATOR NOT YET CONNECTED\n\n<pre>{_pending_modules(update)}</pre>\n\n<b>>> {main.t(lang, 'access')}</b>\n{main.t(lang, 'days7')}\n{main.t(lang, 'days14')}\n{main.t(lang, 'days30')}'
...
async def render_home(update, context, edit: bool=True):
    ...
    text = f'<b>NEURAL GOLD v3.2 // OPERATOR CONSOLE</b>\n{'━' * 28}\nOPERATOR: <b>{main._safe_user_name(user)}</b>\n{ts.stamp()}\n\n{ts.boot(granted=granted)}\n{(main.t(main._lang(update), 'premium_active') if granted else '[ ACCESS ]: PENDING // CLEARANCE REQUIRED')}\n\n<i>{main.t(main._lang(update), 'home_pitch')}</i>\n\n<b>>> {main.t(main._lang(update), 'select_module')}</b>'
