"""Fulfillment recovery worker (fulfillment ops): periodic stale-claim recovery + admin alerting.

Dijadwalkan via APScheduler di app.py lifespan (setiap 60 detik, max_instances=1).
Normal payment -> 0 admin action. Hanya payment yang GAGAL pemulihan otomatis
(attempts >= MAX) yang memicu alert ke admin.
"""
...
from whop_webhook_phase2 import MAX_FULFILLMENT_ATTEMPTS, STALE_FULFILLMENT_MINUTES, recover_stale_fulfillments
...
async def recovery_job(telegram_app) -> dict:
    """Async job: recover, lalu alert admin untuk yang exhausted (hanya exception queue)."""
