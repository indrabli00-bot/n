...
-> stale cache). No MT5 and no fabricated prices. Signal analysis can use the
SMC candle engine merged from the audited uploaded bot when TwelveData is
configured; otherwise the existing deterministic projection remains available.
"""
...
async def fetch_candles(interval: str, outputsize: int = 100) -> list[dict[str, Any]] | None:
    """Fetch XAU/USD candles from TwelveData for the SMC engine."""
...
async def get_smc_signal() -> dict[str, Any] | None:
    """Return an audited SMC signal from live 5M + 15M candles."""
    candles_5m, candles_15m = await __import__("asyncio").gather(
        fetch_candles("5min", 100),
        fetch_candles("15min", 100),
    )
...
def _simulate_technical_indicators(price: float, change_pct: float) -> dict[str, Any]:
    """Deterministic projection fallback for installations without candle data."""
    ...
    return {
        ...
        "macd_signal": round(phase + offsets[4] * 0.6, 2),
...
def _determine_signal(price: float, indicators: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "liquidity": liquidity, "volatility": volatility, "momentum": momentum,
    }
