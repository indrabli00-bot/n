...
The production entry point is `app.py`. `main.py` is the **single canonical customer Telegram UI/controller**. `app.py` owns HTTP/webhook transport; service modules own market data, authentication, database, and Whop operations. No second customer UI contract is installed at runtime.
...
4. Build the Telegram application from the canonical `main.py` controller.
...
2. `main.py` handles the command and owns the customer UI.
3. Inactive customer sees the premium access screen.
4. Customer chooses 7D / 14D / 30D.
5. The signed checkout link creates a real Whop checkout.
6. `payment.succeeded` is verified by signature and allow-listed plan ID.
7. Fulfillment activates the Telegram account atomically and idempotently.
8. Customer notification is delivered independently; failed notification remains recoverable.
